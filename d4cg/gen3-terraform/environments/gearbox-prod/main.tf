...
terraform {
  backend "s3" {
    ...
    bucket = "cdis-state-ac471835990085-gen3" # Update to represent your environment
    # The location of the Terraform state file within the bucket. Notice the bucket has to exist beforehand.
    key     = "gen3-terraform-gearbox/terraform.tfstate" # Update to represent your environment    
    encrypt = "true"
...
locals {
  ...
  availability_zones = ["us-east-1a", "us-east-1b", "us-east-1c"] # ex. ["us-east-1a", "us-east-1c", "us-east-1d"]
  ...
  hostname = "gearbox.pedscommons.org"
  ...
  revproxy_arn = "arn:aws:acm:us-east-1:471835990085:certificate/"
  ...
  user_yaml_bucket_name = "gearbox-test-user-yaml"
  ...
  prod_promotion_role_arn = ""
  staging_account_id      = ""
  is_gearbox_staging      = false
  is_gearbox_prod         = true
  ### Cognito setup
...
module "gen3" {
  ...
  useryaml_s3_path        = "s3://${local.user_yaml_bucket_name}/prod/user.yaml"
...
module "gearbox" {
  source          = "./../../modules/gearbox"
  gearbox_enabled = true
  vpc_name        = local.vpc_name
  # hostname                   = local.hostname
  aurora_username = module.commons.aurora_cluster_master_username
  aurora_password = module.commons.aurora_cluster_master_password
  # gearbox_g3auto_allowed_issuers = ["http://fence-service/,https://${local.hostname}/user"]
